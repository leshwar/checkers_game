...
function load(board, options, isP1AI, isP2AI) {
	if (!board || !board.innerHTML || !options || !options.innerHTML) {
		return new Game([],false,false);
	}
	...
	if (isP1AI) {
		...
		timeOut2 = setTimeout(function(aiMove) {move(aiMove.start, aiMove.end, game);}, move_delay, aiMove);
	}
...
function clickEvent(index) {
	...
	if ((game.isP1Turn && game.isP1AI) || (!game.isP1Turn && game.isP2AI)) {
		return;
	}
	
	if (index < 0 || index >= game.squares.length) {
		return;
	}
...
function move(start, end, board) {
	...
	if (otherSkip) {
		...
		if (!isAI) {
			...
			timeOut2 = setTimeout(function(aiMove) {move(aiMove.start, aiMove.end, board);}, move_delay, getAISkip(end,board));
		}
		board.selected = end;
...
function switchPlayer(start, end, board) {
	...
	if (isAI) {
		...
		if (aiMove) {
			timeOut2 = setTimeout(function(aiMove) {move(aiMove.start, aiMove.end, board);}, move_delay, aiMove);
		} 
		else {
			gameover();
		}
	...
	* is out of bounds, the value of that square in the array returned is null. */
	...
	this.getBlackCheckers = function() {
		...
		for (var i=0; i<squares.length; i++) {
			if (squares[i].isBlackChecker()) {
				checkers.push(squares[i]);
			}
		}
		return checkers;
	}
...
function Square(x, y, id) {
	...
	this.isEnemy = function(test) {
		...
		if (this.isBlackChecker()) {
			return test.isWhiteChecker();
		}
